...
import {
  Box, Typography, Button, Divider, LinearProgress, Tabs, Tab, FormLabel, MenuItem, RadioGroup, FormControlLabel, Radio, Checkbox
} from '@mui/material';
...
function CollectionTypeAddTextField(props) {

  const { handleClose, languageList, isLocalEnable, setFields, fieldType, fieldTypeName,
    editdata, selectedLanguage, fieldNames, setFieldNames } = props
  ...
  const [localDefaultRows, setLocalDefaultRows] = useState(
    editdata &&
      editdata.fieldData.field_settings.default_value &&
      typeof editdata.fieldData.field_settings.default_value === 'object' &&
      isLocalEnable ?
      getEditLocalRows(editdata.fieldData.field_settings.default_value) :
      [{ language: 'en', label: '' }])
  const [localDefaultErrors, setLocalDefaultErrors] = useState([])
  ...
  const addLocalDefaultRow = () => {
    setLocalDefaultRows((prevState) => [...prevState, { language: '', label: '' }])
  }
  ...
  const deleteLocalRow = (index) => {
    ...
    deleteLocalDefaultRow(index)
  }

  const deleteLocalDefaultRow = (index) => {
    let tmpArr = [...localDefaultRows]
    tmpArr.splice(index, 1)
    setLocalDefaultRows(tmpArr)
  }
  ...
  const handleLanguageChange = (index, event) => {
    ...
    handleLanguageDefaultChange(index, event)
  }

  const handleLanguageDefaultChange = (index, event) => {
    const tempObj = [...localDefaultRows]
    tempObj[index].language = event.target.value
    setLocalDefaultRows(tempObj)
  }
  ...
  const handleChangeIndexLanguageDefault = (index, event) => {
    const tempObj = [...localDefaultRows]
    tempObj[index].label = event.target.value
    setLocalDefaultRows(tempObj)
  }
  ...
  const validate = values => {
    ...
    // if (isLocalEnable && localDefaultRows.length === 0)
    //   errors.languagesDefValue = <FormattedMessage {...messages.localReq} />
    return errors;
  };
  const formik = useFormik({
    initialValues: {
      name: editdata && editdata.fieldData ? getEditEnglishName(editdata.fieldData.localized_texts, editdata.fieldData.name) : '',
      defValue: editdata?.fieldData?.field_settings?.default_value?.en ?? editdata?.fieldData?.field_settings?.default_value ?? '',
      minLength: editdata?.fieldData?.field_settings?.min_length ?? '',
      maxLength: editdata?.fieldData?.field_settings?.max_length ?? '',
      type: editdata?.fieldData?.field_sub_type ?? 'short',
    },
    validate,
    onSubmit: async values => {
      const languageValidateStatus = validateLanguage(localRows)
      // const languageValidateDefStatus = validateLanguage(localDefaultRows)
      if (languageValidateStatus.length > 0) {
        setLocalErrors(languageValidateStatus)
        // setLocalDefaultErrors(languageValidateDefStatus)
        return
      }
      ...
      const resObj = {
        ...
        fieldData: {
          ...
          field_settings: {
            default_value: isLocalEnable ? getLocationObject(localDefaultRows) : { en: values.defValue },
          ...
          >
            <Box width="100%">
              ...
              <RadioGroup aria-label="access" size="small" name="type" row value={formik.values.type} onChange={formik.handleChange}>
                <FormControlLabel classes={{ label: classes.mediumLabel }} value={"short"} control={<Radio size='small' />} label={<FormattedMessage {...messages.typeShort} />} />
                <FormControlLabel classes={{ label: classes.mediumLabel }} value={"long"} control={<Radio size='small' />} label={<FormattedMessage {...messages.typeLong} />} />
              </RadioGroup>
              {
                isLocalEnable
                  ?
            ...
            <Box width="100%">
              {
                isLocalEnable ?
                  <Box>
                    <Box sx={classes.localHeadBar}>
                      <Box>
                        <Typography sx={classes.localText}><FormattedMessage {...messages.defValue} /></Typography>
                      </Box>
                      <Box>
                        {
                          // !(editdata?.fieldData?.id) &&
                          // <AddCircle color="secondary" sx={classes.addCircleIcon} onClick={addLocalDefaultRow} />
                        }
                      </Box>
                    </Box>
                    {
                      formik.touched.name && formik.errors.languagesDefValue
                        ?
                        <FormLabel sx={classes.errorLabel}>
                          {formik.errors.languagesDefValue}
                        </FormLabel>
                        :
                        ''
                    }
                    {
                      localDefaultRows.map((row, index) =>
                        <Box key={"local__def_row_key_" + index} display="flex" alignItems="center">
                          <Box sx={classes.formControl}>
                            <TextField
                              variant="filled"
                              label={<FormattedMessage {...messages.language} />}
                              margin="normal"
                              error={formik.touched.name && localDefaultErrors && localDefaultErrors[index] && localDefaultErrors[index].language && localDefaultErrors[index].language !== ""}
                              helperText={(localDefaultErrors && localDefaultErrors[index]?.language) ? localDefaultErrors[index].language : ""}
                              fullWidth
                              name={"language_label" + index}
                              size="small"
                              // disabled={editdata?.fieldData?.id}
                              ...
                              select
                              value={row.language}
                            // onChange={handleLanguageDefaultChange.bind(this, index)}
                            >
                              {
                                languageList.map(lang => <MenuItem key={lang.value} value={lang.value}>{lang.name}</MenuItem>)
                              }
                          ...
                          <Box marginX="15px" width="310px">
                            <TextField
                              ...
                              label=""
                              margin="normal"
                              error={formik.touched.name && localDefaultErrors && localDefaultErrors[index] && localDefaultErrors[index].label && localDefaultErrors[index].label !== ""}
                              helperText={(localDefaultErrors && localDefaultErrors[index]?.label) ? localDefaultErrors[index].label : ""}
                              fullWidth
                              name={"language_label" + index}
                              size="small"
                              type="text"
                              ...
                              autoComplete={"language_label" + index}
                              inputProps={{ lang: row.language }}
                              onChange={handleChangeIndexLanguageDefault.bind(this, index)}
                              value={row.label}
                            />
                          </Box>
                          {
                            // !(editdata?.fieldData?.id) &&
                            // <DeleteOutline color="secondary" sx={classes.addCircleIcon} onClick={deleteLocalDefaultRow.bind(this, index)} />
                          }
                        </Box>
                      )
                    }
...
CollectionTypeAddTextField.propTypes = {
...
export default CollectionTypeAddTextField;
