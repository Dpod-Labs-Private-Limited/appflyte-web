...
const getAgentAPiTokenData = async () => {

    const getAllAgentAPiTokenData = async () => {
        const tempArr = []
        let last_evaluated_key = null
        try {
            do {
                const response = await AgentApiToken.getAllAgentTokens(last_evaluated_key);
                if (response.data) {
    ...
    return getAllAgentAPiTokenData();
};

export default getAgentAPiTokenData;