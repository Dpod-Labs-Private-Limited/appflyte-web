...
const getExtractionFileByFileId = async (file_id) => {
    try {
        const tempArr = [];
        let last_evaluated_key = null;
        do {
            const response = await TemplateExtractionApi.getExtractionFileByFileId(file_id, last_evaluated_key);
            if (response.data) {
...
export default getExtractionFileByFileId;