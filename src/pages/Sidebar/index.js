...
function Sidebar() {
  ...
  const menuItems = [
    { icon: IconSvg.flagIcon, label: 'Get Started', path: 'home' },
    { icon: IconSvg.spaceIcon, label: 'Spaces', path: 'spaces' },
    { icon: IconSvg.projectIcon, label: 'Projects', path: 'projects' },
    { icon: IconSvg.insightIcon, label: 'Insights', path: 'insights' },
    { icon: IconSvg.cloudDataIcon, label: 'Sources', path: 'sources' },
    { icon: IconSvg.settingsIcon, label: 'Settings', path: 'settings' },
    { icon: IconSvg.launchIcon, label: 'Launch', path: 'launch' },
    { icon: IconSvg.logsIcon, label: 'Logs', path: 'logs' },

    { icon: IconSvg.flagIcon, label: 'Get Started', path: 'service_home' },
    { icon: IconSvg.documentTypesIcon, label: 'Document Types', path: 'document_types' },
    { icon: IconSvg.connectorsIcon, label: 'Connectors', path: 'connectors' },
    { icon: IconSvg.extractionFilesIcon, label: 'All Documents', path: 'all_documents' },
    // { icon: IconSvg.sourcesIcon, label: 'Sources', path: 'data_source' },
    { icon: IconSvg.datasetsIcon, label: 'Datasets', path: 'data_sets' },
    { icon: IconSvg.aiAppIcon, label: 'AI App Creator', path: 'app_creator' }
  ];

  const main_sidebar_order = [
    'service_home', 'insights', 'sources',
    'document_types', 'all_documents', 'connectors', 'data_sets', 'app_creator',
    'settings', 'logs', 'launch',
  ];
  ...
  const filteredMenuItems = () => {
    ...
    if (selectedOrganization && selectedService && selectedWorkspace && selectedProject) {
      ...
      const filteredMenuItemsList = menuItems.filter(item => allowedLabels.has(item.path));
      items = filteredMenuItemsList.slice().sort((a, b) => {
  ...
  const handlePageRouting = (path) => {
    ...
    const engine_name = selectedProject?.payload?.configuration?.engine_name ?? null;
    ...
    switch (path) {
      ...
      case 'insights':
        const selected_project_id = selectedProject?.payload?.__auto_id__ ?? null
        navigate(`/spaces/${workspace_id}/projects/${selected_project_id}/insights`)
        break;
  ...
  return (
    ...
    <nav style={styles.sidebar}>
      ...
      <Box sx={{
        ...
        {(selectedOrganization && process.env.REACT_APP_IS_SFS_INSTANCE === "true") &&
          <Box
            sx={{
              display: 'flex', justifyContent: 'center', flexDirection: 'column',
              alignItems: 'center', textAlign: 'center', cursor: 'pointer',
              backgroundColor: (!permissionStatus && !billingStatus && apiKeyStatus) && '#007bff', borderRadius: '10px', padding: '10px 20px 10px 20px', margin: '10px', marginTop: '2px',
              '&:hover': {
          ...
          >
            <ReactSVG src={IconSvg.authenticationIcon}
              beforeInjection={(svg) => {
